...
-opaque dictionary() :: ec_implements:type(?MODULE).
-type key() :: term().
-type value() :: term().
...
-spec new(dictionary() | module()) -> dictionary().
new(ModuleName) when is_atom(ModuleName) ->
    ec_implements:new(ModuleName, ModuleName:new(), ?MODULE);
new(Object) ->
    ec_implements:do_mutate(Object,
			    fun(Module, _Data) ->
				    Module:new()
			    end).
...
-spec has_key(dictionary(), key()) -> boolean().
has_key(Object, Key) ->
    ec_implements:do(Object, fun(Module, Data) ->
				     Module:has_key(Data, Key)
			     end).
...
-spec get(dictionary(), key()) -> value().
get(Object, Key) ->
    ec_implements:do(Object,
		     fun(Module, Data) ->
			     Module:get(Data, Key)
		     end).
...
-spec remove(dictionary(), key()) -> dictionary().
remove(Object, Key) ->
    ec_implements:do_mutate(Object,
			    fun(Module, Data) ->
				    Module:remove(Data, Key)
			    end).
...
-spec has_value(dictionary(), value()) -> boolean().
has_value(Object, Value) ->
    ec_implements:do(Object,
		     fun(Module, Data) ->
			     Module:has_value(Data, Value)
		     end).
